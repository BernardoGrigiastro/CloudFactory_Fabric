...
//
fun fuelBar(blockEntity: BaseBlockEntity, index: Int): WCustomBar {
    val properties = blockEntity.guiSyncableComponent ?: error("$blockEntity does not provide gui_syncable component")
    val fuel = WCustomBar(UNLIT_TEXTURE_ID, LIT_TEXTURE_ID, { properties.get<CraftingComponent>(index).fuelTime }, { properties.get<CraftingComponent>(index).maxFuelTime  }, WCustomBar.Direction.UP)
    fuel.setSize(14, 14)
    return fuel
}
//
...
fun processBar(blockEntity: BaseBlockEntity, index: Int): WCustomBar {
    val properties = blockEntity.guiSyncableComponent ?: error("$blockEntity does not provide gui_syncable component")
    val process = object : WCustomBar(RIGHT_PROCESS_EMPTY, RIGHT_PROCESS_FULL, { properties.get<CraftingComponent>(index).progressTime }, { properties.get<CraftingComponent>(index).maxProgressTime }, Direction.RIGHT) {

        override fun addTooltip(tooltip: TooltipBuilder?) {
            val progress = properties.get<CraftingComponent>(index).progressTime
